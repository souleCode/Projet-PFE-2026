...
from .yolo_fusion import run_detection_with_fusion  # ← NOUVELLE IMPORT
...
class DetectView(APIView):
    ...
    Détection EPI avec pipeline Main + Extra + Fusion
    """
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({'detail': f'Fichier image invalide: {str(e)}'}, status=400)
        ...
        # ═══════════════════════════════════════════════════════════
        # LANCER LA DÉTECTION AVEC FUSION
        # ═══════════════════════════════════════════════════════════
        try:
            detections, stats = run_detection_with_fusion(image_bgr)
        except Exception as e:
            return Response({'detail': f'Erreur détection : {str(e)}'}, status=500)
